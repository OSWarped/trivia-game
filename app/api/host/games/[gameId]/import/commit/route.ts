...
function validateAndNormalizeImport(
    ...
    const tag =
        raw.tag === undefined || raw.tag === null ? null : asTrimmedString(raw.tag);
    ...
    raw.rounds.forEach((roundValue, roundIndex) => {
        ...
        if (roundType === RoundType.TIME_BASED && timeLimit === null) {
            pushError(
                errors,
                `${roundPath}.timeLimit`,
                'timeLimit is recommended for TIME_BASED rounds'
            );
        }

        if (roundType === RoundType.WAGER && wagerLimit === null) {
            warnings.push(`${roundPath}.wagerLimit: wagerLimit is recommended for WAGER rounds`);
        }
...
export async function POST(
    ...
    try {
        ...
        console.error('Failed to commit game import', { message });

        return NextResponse.json(
            {
                ok: false,
                error: 'Failed to import game content.',
            },
            { status: 500 }
        );
    }
