...
export default function AnswerRevealPage(): JSX.Element {
    ...
    const initializePage = useCallback(async () => {
        ...
        try {
            ...
            console.error('Failed to initialize answer reveal page:', error);
            setLoadError('Failed to restore answer reveal session.');
        } finally {
    ...
    useEffect(() => {
        ...
        const raw = sessionStorage.getItem(`trivia:${gameId}:answerReveal`);
        if (!raw) return;

        try {
            const parsed = JSON.parse(raw) as AnswerRevealPayload;
            setReveal(parsed);
        } catch (err) {
            console.error('Failed to parse answer reveal payload', err);
        }
    }, [gameId]);
    ...
    useEffect(() => {
        ...
        const raw = sessionStorage.getItem(`trivia:${gameId}:answerReveal`);
        console.log('ANSWER REVEAL STORAGE RAW', raw);

        if (!raw) return;

        try {
            const parsed = JSON.parse(raw) as AnswerRevealPayload;
            console.log('ANSWER REVEAL PARSED', parsed);
            setReveal(parsed);
        } catch (err) {
            console.error('Failed to parse answer reveal payload', err);
        }
    ...
    return (
        <div className="min-h-screen bg-white p-6">
            <div className="mx-auto max-w-4xl">
                <h1 className="mb-6 text-3xl font-bold text-orange-800">
                    ✅ Answer
                </h1>
                ...
                <div className="mb-6 rounded bg-white p-6 shadow">
                    ...
                    {teamName ? (
                        <p className="mt-2 text-sm text-gray-500">
                            Team:{' '}
                            <span className="font-medium text-orange-800">{teamName}</span>
                        </p>
                    ) : null}
                        ...
                        {reveal ? (
                            <>
                                ...
                                <div className="mt-8 text-left">
                                    ...
                                    <ul className="mt-3 space-y-3">
                                        {reveal.correctAnswers.map((answer) => (
                                            <li
                                                key={answer}
                                                className="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-lg font-medium text-green-800"
                                            >
                                                {answer}
                                            </li>
                                        ))}
                            ...
                            <>
                                <h3 className="text-2xl font-bold text-slate-900">
                                    The host is revealing the answer
                                </h3>

                                <p className="mx-auto mt-4 max-w-2xl text-base text-slate-600">
                                    No reveal data is available yet.
                                </p>
                            </>
                        )}
