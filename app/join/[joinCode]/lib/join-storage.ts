import type { StoredTeamSession } from './join-types';
...
export function ensureStoredDeviceIdentity(): StoredDeviceIdentity {
  if (typeof window === 'undefined') {
    throw new Error('Device identity is only available in the browser.');
  }

  const existing = getStoredDeviceIdentity();
  ...
  localStorage.setItem(`${STORAGE_PREFIX}:device`, JSON.stringify(created));
  return created;
}
